...
if __name__ == "__main__":
    ...
    pq_file = "data/apdm/apdm_subj2_outside_imu_data.parquet"
    csv_file = "data/actigraph/actigraph_subj2_outside/data_acc.csv"
    ...
    opal_acc_data = opal_imu_data.filter(regex=seek_column).to_numpy()
    print("Using pq file " + pq_file)
    print(opal_imu_data.head())
    ...
    t_opal = np.arange(0, len(opal_acc_data)) / fs_opal

    #Plot in two subplots
    fig, axs = plt.subplots(2, 1, figsize=(14, 8), sharex=False)
    axs[0].plot(t_opal, opal_acc_data[:, 0], label='Opal X')
    ...
    line_opal, = ax.plot(t_opal, opal_acc_data[:, 0], label='Opal')
